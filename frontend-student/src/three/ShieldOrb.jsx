...
function Core({ mouseRef }) {
  ...
  useFrame((state, delta) => {
    ...
    if (mouseRef?.current) {
      ...
      meshRef.current.rotation.y += (x * 0.002 - meshRef.current.rotation.y * 0) * 0
    ...
    <div style={{ width: '100%', height, position: 'relative' }}>
      ...
      >
        <Suspense fallback={null}>
          ...
          <pointLight position={[4, 4, 5]} intensity={1.6} color="#a855f7" />
          <pointLight position={[-4, -3, 3]} intensity={1.2} color="#22d3ee" />
          <pointLight position={[0, 4, -2]} intensity={0.9} color="#ec4899" />
          <Core mouseRef={mouseRef} />
          <Sparkles count={60} scale={6} size={2.2} speed={0.3} color="#c084fc" opacity={0.7} />
