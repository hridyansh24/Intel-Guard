...
- **LLM:** Provider-agnostic (OpenAI/Anthropic/Gemini) — configured via `.env`. Currently using Anthropic Codex API (Sonnet 4.5 primary, Haiku 4.5 mini) with prompt caching.
...
- Using Anthropic Codex API: `Codex-sonnet-4-5` (primary) and `Codex-haiku-4-5` (mini) with prompt caching enabled
